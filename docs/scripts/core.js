// core.js v1.2 2025-07-19T23:00:00Z

let tooltipDefinitions = {};
let tagDefinitions = {};
let categoryColors = {}; // Map for category colors
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeTooltips() {
    ...
    // Create or reuse tooltip container
    ...
    // Clear previous content, setup icon and content wrapper
    tooltip.innerHTML = "";
  ...
  fetch("/data/tooltips.json")
    ...
    .catch((error) => {
      console.warn("Could not load tooltip definitions, proceeding without them.", error);
      // Initialize anyway (empty tooltips)
      initializeTooltips();
    });
  ...
  fetch("/data/tags.json")
    ...
    .then((data) => {
      ...
      // Apply colors to existing .tag elements
    ...
    .catch((error) => {
      console.warn("Could not load tag definitions, proceeding without them.", error);
      initializeTagTooltips();
    });

  // Run initialization of other features regardless of fetch results
  initAll();
});